package com.etiya.cartservice.outbox;
 ...
 *
 * <p>The business layer records a row here in the same transaction that writes the cart, instead of
 * publishing to Kafka directly. A {@link OutboxMessageRelay polling relay} later reads
 * {@link OutboxStatus#PENDING} rows and forwards the {@link #payload} (already serialized JSON) to
 * the {@link #destination} binding, flipping the row to {@link OutboxStatus#PUBLISHED} once the
 * broker accepts it.</p>
 */
@Entity
...
public class OutboxEvent {
    ...
    /** Domain aggregate the event belongs to, e.g. {@code "Cart"}. */
    @Column(nullable = false)
    private String aggregateType;

    /** Identifier of the aggregate instance, e.g. the cart id. */
    @Column(nullable = false)
    private String aggregateId;

    /** Logical event name, e.g. {@code "CartCheckedOut"}. */
    @Column(nullable = false)
